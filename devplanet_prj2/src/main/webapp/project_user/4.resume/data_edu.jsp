<%@page import="kr.co.sist.user.vo.EducationAllVO"%>
...
try{
	List<EducationAllVO> list = rmDAO.selectEducation(rNum); 
...
//1. JSONArray 생성
JSONArray jsonEduArr = new JSONArray();


//2. 데이터베이스에서 조회한 결과로 JSONObject 생성하고, JSONArray에 추가
JSONObject jsonEduTemp = null;

if(!list.isEmpty()){
	

	for(EducationAllVO eaVO : list){
		//JSONObject 생성
		jsonEduTemp = new JSONObject();
		
		//생성된 JSONObject에 속성값을 넣는다.
		jsonEduTemp.put("num",eaVO.getNum());
		jsonEduTemp.put("schoolType",eaVO.getSchoolType());
		jsonEduTemp.put("schoolName",eaVO.getSchoolName());
		jsonEduTemp.put("period",eaVO.getPeriod());
		jsonEduTemp.put("major",eaVO.getMajor());
		jsonEduTemp.put("gpa",eaVO.getGpa());
		
		//값을 가진 JSONObject을 JSONArray에 할당
		jsonEduArr.add(jsonEduTemp);
		
	}//end for
//3. JSONArray 출력
out.print(jsonEduArr.toJSONString());
 }//end if
